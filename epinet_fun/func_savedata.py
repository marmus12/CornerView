...
def display_current_output(train_output, traindata_label, iter00, directory_save, split, corner="_"):
    '''
        display current results from EPINET 
        and save results in /current_output
    '''
    sz=len(traindata_label)
    train_output=np.squeeze(train_output,-1)
    ...
    imageio.imsave(directory_save+'/'+corner+"_"+split+'_epoch%05d.jpg' % (iter00), np.squeeze(train_output482_all))
